...
import PixelBirthdayCake from './components/PixelBirthdayCake';
import { BLESSING_THEMES, DEFAULT_BIRTHDAY_IMAGE, CLICK_SOUND_URL, DEFAULT_MEMORY_PHOTOS } from './data';
import { MemoryPhoto } from './types';
import { saveCardToLocal, loadCardFromLocal } from './utils/storage';
import { getSeedCard } from './data/seedCards';
...
export default function App() {
  ...
  const [birthdayName, setBirthdayName] = useState(() => seedCard?.name || '最特别的孩子');
  ...
  useEffect(() => {
    ...
    const applyCardData = (card: any) => {
      if (!card) return;
      if (card.name) {
        setBirthdayName(card.name);
        setInputName(card.name);
        setTempName(card.name);
      }
      if (card.age) {
        setBirthdayAge(card.age);
        setInputAge(card.age);
      }
      if (card.themeIndex !== undefined) {
        setSelectedThemeIndex(card.themeIndex);
      }
      if (card.activeMascotId !== undefined) {
        setActiveMascotId(card.activeMascotId);
      }
      if (card.mascotConfig !== undefined) {
        setMascotConfig(card.mascotConfig);
      }
      if (card.customTitle !== undefined) setCustomTitle(card.customTitle);
      if (card.customParagraphs !== undefined) setCustomParagraphs(card.customParagraphs);
      if (card.customSignature !== undefined) setCustomSignature(card.customSignature);
      if (card.customDate !== undefined) setCustomDate(card.customDate);
      if (card.customThemeNames !== undefined) setCustomThemeNames(card.customThemeNames);
    ...
    // 0. Instant offline seed card fallback
    if (seedCard) {
      applyCardData(seedCard);
    }

    // 0.1 Zero-server cross-device payload fallback (reads encoded data if present)
    const dParam = urlParams.get('d') || (window.location.hash ? new URLSearchParams(window.location.hash.replace(/^#/, '')).get('d') : null);
    if (dParam) {
      try {
        const jsonStr = decodeURIComponent(escape(atob(decodeURIComponent(dParam))));
        const decoded = JSON.parse(jsonStr);
        applyCardData(decoded);
        saveCardToLocal(currentCardId, decoded);
      } catch (err) {
        console.warn('URL payload decode error:', err);
      }
    }

    // 1. Local-First Caching: Instantly load configuration from IndexedDB / localStorage
    loadCardFromLocal(cardId).then((cached) => {
      if (cached) {
        applyCardData(cached);
      }
    }).catch(err => console.warn('Failed to parse local cache', err));
    ...
    fetch(`/api/cards/${cardId}`)
      ...
      .then((data: any) => {
        if (data && data.success && data.card) {
          ...
          saveCardToLocal(currentCardId, data.card);
        } else {
          // Fallback to seed card if cloud returned empty
          const fallback = getSeedCard(cardId);
          if (fallback) {
            ...
            fetch('/api/cards/latest')
              .then(r => r.json())
              .then((latestData: any) => {
                if (latestData && latestData.success && latestData.card) {
                  applyCardData(latestData.card);
                  saveCardToLocal(currentCardId, latestData.card);
                }
              })
              .catch(() => {});
          }
      ...
      .catch(err => {
        console.log('Notice: using local / seed card fallback', err);
        const fallback = getSeedCard(cardId);
        if (fallback) {
          applyCardData(fallback);
        }
  ...
  const [inputName, setInputName] = useState(() => seedCard?.name || '最特别的孩子');
  const [inputAge, setInputAge] = useState(() => seedCard?.age || 18);
  ...
  const handleShareLink = () => {
    ...
    saveCardToLocal(cardId, cardPayload);
...
// Save state to server & local storage on user change
  useEffect(() => {
    ...
    // 1. Instantly save to IndexedDB & localStorage
    saveCardToLocal(cardId, cardPayload);
  ...
  }, [birthdayName, birthdayAge, wishesList, selectedThemeIndex, activeMascotId, mascotConfig, customTitle, customParagraphs, customSignature, customDate, customThemeNames, photos]);
      ...
      <AnimatePresence mode="wait">
        ...
        {introSequence ? (
          ...
          >
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[#C9A9E9]">向粉紫像素星河许个愿，吹熄蛋糕上的蜡烛后即会出发</p>
                </div>
              </div>

              {/* Retro Birthday Cake with Candle on top */}
              <div className="flex justify-center items-center py-3 relative border-2 border-dashed border-[#7B5EA7]/40 bg-black/30 p-2 overflow-hidden">
                <div className="flex flex-col items-center relative">
                  ...
                  <AnimatePresence>
                    {floatingWishText && (
                      <motion.div
                        ...
                        animate={{ y: -80, scale: [1, 1.15, 0.9], opacity: [0, 1, 1, 0] }}
                        exit={{ opacity: 0 }}
                        transition={{ duration: 1.2, ease: "easeOut" }}
                        className="absolute -top-6 text-center flex flex-col items-center pointer-events-none z-30"
                      >
                        <div className="text-[10px] bg-[#FF8FA3] border-2 border-white px-2.5 py-0.5 text-[#24133c] font-bold font-pixel whitespace-nowrap shadow-md">
                          ⭐ {floatingWishText} ⭐
                        </div>
                  ...
                  {/* Pixel Birthday Cake with glowing interactive candle on top */}
                  <PixelBirthdayCake
                    isLit={isCandleLit}
                    onBlowout={() => {
                      ...
                      playClickSound();
                      setIsCandleLit(false);
                      setWishNotification(`💨 呼…… 愿望伴随着烛火吹熄，在寿星【${birthdayName}】的心田里悄然成真！`);
                      setTimeout(() => {
                        setIsCandleLit(true);
                        setWishNotification('');
                      }, 5500);
                ...
                {isCandleLit && (
                  <div 
                    ...
                    className="absolute right-3 top-3 text-[8px] text-[#F7D070] bg-[#7B5EA7]/30 hover:bg-[#7B5EA7]/60 transition-colors cursor-pointer px-1.5 py-0.5 border border-[#C9A9E9]/40 flex items-center gap-1 leading-none select-none"
                    title="点击吹熄蛋糕蜡烛"
                  >
            ...
            {/* Player tips */}
            <div className="text-center text-[10px] text-[#C9A9E9]/70 font-pixel py-1 select-none">
            ...
            {/* Retro Aesthetic Footer Credits */}
            <div className="w-full text-center flex flex-col items-center gap-1.5 pt-4 pb-2 text-[10px] font-pixel text-[#C9A9E9]/60 select-none">
              <div className="flex items-center justify-center gap-2">
                <span className="text-[#FF8FA3]">♥</span>
                <span>愿你今夜被像素星辰温柔拥抱</span>
                <span className="text-[#FF8FA3]">♥</span>
              </div>
              <div className="text-[9px] tracking-widest text-[#C9A9E9]/40 font-pixel">
                Lagrange ✦ 小红书
              </div>
            </div>
